  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-semibold text-cream">{t('moodTracker')}</h3>
          <p className="text-white/40 text-xs">{t('trackEmotionalJourney')}</p>
        </div>
      </div>

      {/* Today's mood */}
      <div className="glass-panel p-5">
        <p className="text-white/50 text-sm mb-3">{t('logTodaysMood')}</p>
          ...
          {moodOptions.map((mood) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-medium">{t(mood.label.toLowerCase())}</span>
              </motion.button>
            )
          })}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="glass-panel p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-white/50 text-xs">{t('averageMood')}</span>
            </div>
            <p className="text-2xl font-semibold text-cream">{avgMood}</p>
            <p className="text-white/30 text-xs">{t('outOf5')}</p>
          </div>
          <div className="glass-panel p-4">
            <div className="flex items-center gap-2 mb-1">
              <Calendar className="w-4 h-4 text-cosmic-400" />
              <span className="text-white/50 text-xs">{t('daysLogged')}</span>
            </div>
            <p className="text-2xl font-semibold text-cream">{moods.length}</p>
            <p className="text-white/30 text-xs">{t('last30Days')}</p>
          </div>
        </div>
      )}
      ...
      {/* Weekly view */}
      <div className="glass-panel p-5">
        <p className="text-white/50 text-sm mb-4">{t('moodHistory')}</p>
